...
class DrmmForRankData():
    def __init__(self,idlist,traindata,idfdata,y_label):
        super(DrmmForRankData, self).__init__()
        self.idlist = idlist
    ...
    def __getitem__(self, index):
        ...
        label_tensor = torch.tensor(self.y_label[index],dtype = torch.float32)
        return qid,docid,rel_tensor.cuda(),idf_tensor.cuda(),label_tensor.cuda()

class DrmmRankDataloader():
    def __init__(self,
                 bin=0.5,
                 querynum=5,
                 idf_file="",
...
class DRMMrank():
    ...
    def train(self,data,y_label,qid,drmmloader,epoch=1000,max_step=50):
        ...
        train_data = DrmmForRankData(traindata,train_data,idf_data,y_train) 
        ...
        for num in range(int(epoch)):
            ...
            for step, batch in enumerate(tqdm_bar):
                qid,docid,pos_tensor,idf_tensor,label = batch
                pos_value = self.model(pos_tensor, idf_tensor)
                
                criterion = MSELoss()
                loss=criterion(pos_value,label)
            ...
            result_dict = self.predict(val_posdata,drmmloader,test_batchsize=self.batch_size)
    ...
    def predict(self,data,drmmloader,test_batchsize = 16):
        ...
        self.model.eval()
        with torch.no_grad():
            test_data = DrmmForRankData(data,test,idf,label_)
            test_loader = DataLoader(test_data,batch_size=test_batchsize)
            for batch in test_loader:
                qid,docid,histogram_tensor,idf_tensor,_ = batch
